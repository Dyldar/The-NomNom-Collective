...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    // Veritabanı açma veya oluşturma
  ...
  Future<void> _createDB(Database db, int version) async {
    ...
    // Hastalık kısıtlamaları tablosu
    ...
    // Hastalık yasaklı yiyecekler tablosu
  ...
  // Veritabanı sürümü yükseldiğinde yapılacak işlemler
  ...
  Future<void> _insertSampleData(Database db) async {
    ...
    // Hastalık yasaklı yiyecek verisi
  ...
  // Veritabanı bağlantısının doğruluğunu kontrol etme
  ...
  // Hastalık yasaklı yiyecekleri almak için
  ...
  // food_id'yi food_name'e göre almak için yardımcı fonksiyon
  Future<int> _getFoodIdByName(Database db, String foodName) async {
    ...
    if (result.isNotEmpty) {
      return result.first['id'] as int; // id'yi int olarak döndür
    } else {
      return -1; // Eğer sonuç boşsa -1 döndür
    }
  }
}
